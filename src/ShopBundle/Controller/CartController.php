...
use ShopBundle\Entity\factuur_regel;
...
class CartController extends Controller
{
    ...
    public function checkoutAction()
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $userAdress = $em->getRepository('ShopBundle:User')->findOneBy(array('id' => $this->getUser()->getId()));

        // new UserAdress if no UserAdress found...
        if (!$userAdress) {
            $userAdress = new User();
            $userAdress->set($this->getUser()->getId());
        }

        $factuur = new factuur();
        $factuur->setFactuurdatum(new \DateTime("now"));
        $factuur->setKlantnr($this->getUser());
        ...
        if (isset($cart)) {
            ...
            foreach ($cart as $id => $quantity) {
                $regel = new factuur_regel();
                $regel->setFactuurnr($factuur);

                $em = $this->getDoctrine()->getManager();
                $product = $em->getRepository('ShopBundle:artikel')->find($id);

                $regel->setEenheid($quantity);
                $regel->setArtikelnr($product);
